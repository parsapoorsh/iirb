...
parser.add_argument('hostname', type=str, help='Domain/IP address of this server')
parser.add_argument('rsserver', type=str, help='Shadowsocks server outside Iran WITHOUT base64 encode')
parser.add_argument('sport', type=int, default=80, nargs='?', help='This server port for shadowsocks proxy. default is 80')
...
for ptype in ('tcp', 'udp'):
    if os.system(f'iptables -I INPUT -p {ptype} -m {ptype} --dport {args.sport} -j ACCEPT') != 0:
...
try:
    print(f'Share URL: ss://chacha20-ietf-poly1305:{args.spassword}@{args.hostname}:{args.sport}')
    print(
        'Batch export share URL: ss://',
        urlsafe_b64encode(f'chacha20-ietf-poly1305:{args.spassword}@{args.hostname}:{args.sport}'.encode()).decode(),
        sep=''
    )
    loop.run_forever()
